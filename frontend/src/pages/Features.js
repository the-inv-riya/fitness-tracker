...
const FeatureCard = ({ title, description, link }) => {
  return (
    <Card className="mb-4 text-center" style={{ height: '270px' }}>
      <Card.Body>
        <Card.Title style={{ fontWeight: 'bold', fontSize: '1.5rem' }} className="mt-3">{title}</Card.Title>
        <Card.Text style={{ fontSize: '1.2rem' }}>{description}</Card.Text>
        <Link to={link} className="btn btn-primary" style={{ background: '#03346E' }}>
          Learn More
        </Link>
      </Card.Body>
    </Card>
  ...
  return (
    <>
      <Container className="mt-4">
        <h2 style={{ fontWeight: 'bold' }} className="text-center mb-4">Website Features</h2>
